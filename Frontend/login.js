...
class LoginForm1 {
    ...
    handleSignupLink(e) {
        ...
        // Use a small timeout so the animation/notification is visible
        setTimeout(() => {
    ...
    handleSocialLogin(e) {
        ...
        // After brief animation, navigate to backend OAuth endpoint (if implemented)
        setTimeout(() => {
            const backendBase = (window.BACKEND_BASE || '').replace(/\/$/, '');
            const path = provider === 'Google' ? '/v1/auth/google' : '/v1/auth/github';
...
document.addEventListener('DOMContentLoaded', () => {
    // If redirected back from OAuth with tokens in query, capture them for demo
    const params = new URLSearchParams(window.location.search);
    const accessToken = params.get('accessToken');
    const refreshToken = params.get('refreshToken');
    if (accessToken) {
        try {
            // Store tokens (demo only). In production, use secure httpOnly cookies.
            localStorage.setItem('accessToken', accessToken);
            if (refreshToken) localStorage.setItem('refreshToken', refreshToken);
            FormUtils.showNotification('Signed in via OAuth successfully', 'success', document.body);
